...
\author{Jessica de Leeuw, Sam Cymbaluk, Jeff Gibson and Fanping Jiang}

\begin {document}

\maketitle

Artificial Facial Aging through Deep Neural Networks
...
CardStackT is  {What should go here?}
...
$|T| =  {\text{What goes here?}}$\\
$|F| =  {\text{What goes here?}}$\\
cnt\_cards($T$, $F$, $D$, $W$,  {\text{What goes here?}}) = TOTAL\_CARDS\\
...
\hhline{|-|-|}
$c = \mbox{Deck}$&  {What goes here?}\\
\hhline{|-|-|}
$c = \mbox{Waste}$ &  {What goes here?}\\
\hhline{|-|-|}
\end{tabular}
...
\begin{itemize}
\item output:  {What goes here?  The deck moves involves moving a card from
...
\begin{tabular}{|p{3cm}|l|}
\hhline{|-|-|}
$c = \mbox{Tableau}$ & T[$n_0$], T[$n_1$] :=  {What goes here?}\\
\hhline{|-|-|}
$c = \mbox{Foundation}$ & T[$n_0$], F[$n_1$] :=  {What goes here?}\\
\hhline{|-|-|}
\end{tabular}
...
\begin{tabular}{|p{3cm}|l|}
\hhline{|-|-|}
$c = \mbox{Tableau}$ & W, T[$n$] :=  {What goes here?}\\
\hhline{|-|-|}
$c = \mbox{Foundation}$ & W, F[$n$] :=  {What goes here?}\\
\hhline{|-|-|}
\end{tabular}
...
\noindent deck\_mv():
\begin{itemize}
\item transition: D, W :=  {What goes here?}
\item exception:
...
\item output: $out := \text{valid\_tab\_mv} \vee \text{valid\_waste\_mv} \vee
  ...
  n_1: \mathbb{N} |  {\text{What goes here?}}: \text{is\_valid\_tab\_mv}(c, n_0, n_1))$\\

  $\text{valid\_waste\_mv} \equiv (\exists c: \text{CategoryT}, n: \mathbb{N} |
   {\text{What goes here?}} : \text{is\_valid\_waste\_mv}(c, n))$
...
\noindent
$\text{two\_decks}(T, F, D, W) \equiv$  {This function returns True if there
...
\mathit{deck}[ {\text{What goes here?}}])$\\
...
\hhline{|-|-|-|}
$T[n_0].\text{size}() > 0$ & T[$n_1$].size() $>$ 0 &  {What goes here?}\\
\hhline{|~|-|-|}
 & T[$n_1$].size() = 0 &  {What goes here?}\\
\hhline{|-|-|-|}
T[$n_0$].size() = 0 & T[$n_1$].size() $>$ 0 &  {What goes here?}\\
\hhline{|~|-|-|}
& T[$n_1$].size() $=$ 0 &  {What goes here?}\\
\hhline{|-|-|-|}
\end{tabular}\\\\
...
\noindent valid\_tab\_foundation($n_0, n_1$) $\equiv$  {What goes here?  You
  may need a table?}\\
...
\noindent tab\_placeable:  {Complete this specification}\\

\noindent foundation\_placeable:  {Complete this specification}
